...
` 새로운 도전의 경험이 쌓여 새로운 도전을 할 수 있는 힘을 길렀던 소중한 3개월이였습니다. 단순한 CRUD로 이루어진 프로젝트만 했고, 그 테두리에서만 생각을 해왔었는데 완전히 바뀌게되었습니다. webSocket, webRTC, 실시간 상태 관리, FCM 알림,, 모든게 새로웠고 큰 어려움이였지만, 결국은 다 헤처왔습니다. 팀원들과 함께라면 이제는 못할 것이 아무것도 없다는 생각이듭니다. 이러한 기회를 준 Stove, 그리고 우리 팀원 Smooth 모두 감사드립니다.`
